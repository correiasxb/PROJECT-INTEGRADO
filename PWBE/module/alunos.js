...
const studentsOfCourse = function(nameCourse){
    let curso = nameCourse;
    let erro = true;
    let listOfStudents = {};
    let studentOfArray = [];

    alunos.forEach(item => {
        item.curso.forEach(cursoElement =>{
            if(cursoElement.sigla.toUpperCase() == cursoElement.toUpperCase()){
                studentOfArray.push(
                {
                    nome: item.nome,
                    icone: item.foto,
            ...
            listOfStudents.alunos = studentOfArray
        })
    })
    if(erro){
        return false;
...
const getStudentDisc = function(matriculaStudent){
    let studentJson = {}
    let studentArray = []
    let erro = true;
    ...
    alunos.forEach(item =>{
        if(item.matriculaStudent == matriculaStudent){
            studentArray.push({nome: item.nome, foto: item.foto, materias: []})
            erro = false
            item.curso.forEach(curso =>{
                curso.disciplinas.forEach(disciplinas => {
                    studentArray[0].materias.push({nome: disciplina.nome, media: disciplina.media, status: disciplina.status})

                })
            })
        }
    })
    studentJson.aluno = studentArray
    if(erro){
        return false
    }else{
        return studentJson
...
console.log(getStudentDisc(20151001024))